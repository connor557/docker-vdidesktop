...
RUN apt-get update && apt-get install --yes --force-yes --no-install-recommends git wget rsync && apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* && apt-get install --yes --force-yes xubuntu-desktop
