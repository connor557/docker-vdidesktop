...
RUN apt-get update && apt-get install --yes --force-yes --no-install-recommends git wget rsync xubuntu-desktop xscreensaver-extras && apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
